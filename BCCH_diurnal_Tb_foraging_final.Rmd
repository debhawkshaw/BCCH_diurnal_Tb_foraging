...
#### Models with spurious data and two individuals excluded #####

hist(thermal_bird_forage_with_sp_v3$IVI)
model1.2_with_sp <- lmer(IVI ~ scaled.Avg.temp.C + scaled.Daylength.Hr + (1|TransponderHexCode) + (1|date), data = thermal_bird_forage_with_sp_v3, na.action = na.exclude)
summary(model1.2_with_sp)
hist(resid(model1.2_with_sp))
qqnorm(resid(model1.2_with_sp))
qqline(resid(model1.2_with_sp))


hist(thermal_bird_forage_with_sp_v3$median.visit.temperature.C)
model2.2_with_sp <- lmer(median.visit.temperature.C ~ scaled.Avg.temp.C + scaled.Daylength.Hr + (1|TransponderHexCode) + (1|date), data = thermal_bird_forage_with_sp_v3, na.action = na.exclude)
summary(model2.2_with_sp)
hist(resid(model2.2_with_sp))
qqnorm(resid(model2.2_with_sp))
qqline(resid(model2.2_with_sp))
```
...
MCMC_model1 <- MCMCglmm(cbind(median.visit.temperature.C, IVI) ~ trait-1 + trait:scaled.Avg.temp.C +
...
MCMC_model2 <- MCMCglmm(cbind(median.visit.temperature.C, IVI) ~ trait-1 + trait:scaled.Avg.temp.C +
trait:scaled.Daylength.Hr,
random =~ us(trait):TransponderHexCode,
rcov =~ us(trait):units,
family = c("gaussian","gaussian"),
...
MCMC_model3 <- MCMCglmm(cbind(median.visit.temperature.C, IVI) ~ trait-1 + trait:scaled.Avg.temp.C +
trait:scaled.Daylength.Hr,
random =~ us(trait):TransponderHexCode,
rcov =~ us(trait):units,
family = c("gaussian","gaussian"),
...
MCMC_model1_prior2 <- MCMCglmm(cbind(median.visit.temperature.C, IVI) ~ trait-1 + trait:scaled.Avg.temp.C +
trait:scaled.Daylength.Hr,
random =~ us(trait):TransponderHexCode,
rcov =~ us(trait):units,
family = c("gaussian","gaussian"),
...
##############
# Need to run the code below to confirm priors don't change results in these models. 
##############

#### Model without the two individuals whose temperature data look quite different compared to the rest.
MCMC_model2_prior2 <- MCMCglmm(cbind(median.visit.temperature.C, IVI) ~ trait-1 + trait:scaled.Avg.temp.C +
trait:scaled.Daylength.Hr,
random =~ us(trait):TransponderHexCode,
rcov =~ us(trait):units,
family = c("gaussian","gaussian"),
...
MCMC_model3_prior2 <- MCMCglmm(cbind(median.visit.temperature.C, IVI) ~ trait-1 + trait:scaled.Avg.temp.C +
trait:scaled.Daylength.Hr,
random =~ us(trait):TransponderHexCode,
rcov =~ us(trait):units,
family = c("gaussian","gaussian"),
...
#### All individuals, spurious data excluded ####
MCMC_model4 <- MCMCglmm(cbind(mean.body.temperature,total.feeder.visits) ~ trait-1+
...
#### Run without the two individuals whose temperature data look quite different compared to the rest ####
MCMC_model5 <- MCMCglmm(cbind(mean.body.temperature,total.feeder.visits) ~ trait-1+
                            trait:scaled.Avg.temp.C +
                            trait:scaled.Daylength.Hr,
                          random =~ us(trait):TransponderHexCode,
                          rcov =~ us(trait):units,
...
MCMC_model6 <- MCMCglmm(cbind(mean.body.temperature,total.feeder.visits) ~ trait-1+
                            trait:scaled.Avg.temp.C +
                            trait:scaled.Daylength.Hr,
                          random =~ us(trait):TransponderHexCode,
                          rcov =~ us(trait):units,
...
MCMC_model4_prior2 <- MCMCglmm(cbind(mean.body.temperature,total.feeder.visits) ~ trait-1+
                            trait:scaled.Avg.temp.C +
                            trait:scaled.Daylength.Hr,
                          random =~ us(trait):TransponderHexCode,
                          rcov =~ us(trait):units,
...
#### Run without the two individuals whose temperature data look quite different compared to the rest ####
MCMC_model5_prior2 <- MCMCglmm(cbind(mean.body.temperature,total.feeder.visits) ~ trait-1+
                            trait:scaled.Avg.temp.C +
                            trait:scaled.Daylength.Hr,
                          random =~ us(trait):TransponderHexCode,
                          rcov =~ us(trait):units,
...
MCMC_model6_prior2 <- MCMCglmm(cbind(mean.body.temperature,total.feeder.visits) ~ trait-1+
                            trait:scaled.Avg.temp.C +
                            trait:scaled.Daylength.Hr,
                          random =~ us(trait):TransponderHexCode,
                          rcov =~ us(trait):units,
...
MCMC_model7 <- MCMCglmm(cbind(mean.body.temperature,total.feeder.visits) ~ trait-1 +
                            trait:scaled.Avg.temp.C +
...
MCMC_model8 <- MCMCglmm(cbind(median.visit.temperature.C,IVI) ~ trait-1 +
                            trait:scaled.Avg.temp.C +
                            trait:scaled.Daylength.Hr + trait:SexConclusion,
                          random =~ us(trait):TransponderHexCode,
                          rcov =~ us(trait):units,
...
# Daily mean body temperature and daily feeder visits
MCMC_model7_prior2 <- MCMCglmm(cbind(mean.body.temperature,total.feeder.visits) ~ trait-1 +
                            trait:scaled.Avg.temp.C +
                            trait:scaled.Daylength.Hr + trait:SexConclusion,
                          random =~ us(trait):TransponderHexCode,
                          rcov =~ us(trait):units,
...
# Visit body temperature and IVI 
MCMC_model8_prior2 <- MCMCglmm(cbind(median.visit.temperature.C,IVI) ~ trait-1 +
                            trait:scaled.Avg.temp.C +
                            trait:scaled.Daylength.Hr + trait:SexConclusion,
                          random =~ us(trait):TransponderHexCode,
                          rcov =~ us(trait):units,
...
#### Figure 2. Within individual correlations between IVI and body temperature
```{r}
# Plotting with raw data
# Determine mean and sd for each individual
Figure2_data <- data.table(thermal_bird_forage_v2)

means.sd <- Figure2_data[, .(mean.visit.Tb = mean(median.visit.temperature.C, na.rm=TRUE), sd.visit.Tb = sd(median.visit.temperature.C, na.rm=TRUE) , mean.IVI = mean(IVI, na.rm=TRUE), sd.IVI = sd(IVI, na.rm=TRUE)), by= c("TransponderHexCode")] 

Figure2_data_v2 <- merge(Figure2_data, means.sd, by = "TransponderHexCode")

# Standardize visit body temperature and IVI
Figure2_data_v2$centered.visit.Tb <- (Figure2_data_v2$median.visit.temperature.C-Figure2_data_v2$mean.visit.Tb)/Figure2_data_v2$sd.visit.Tb

Figure2_data_v2$centered.IVI <- (Figure2_data_v2$IVI-Figure2_data_v2$mean.IVI)/Figure2_data_v2$sd.IVI
...
Figure2_data_v3 <- Figure2_data_v2 %>%
  slice(sequence)

#### All data points ####
# Figure2 <- ggplot(data = Figure2_data_v2, aes(x = centered.visit.Tb, y = centered.IVI))+geom_point()+theme_classic()+xlab(expression("within-individual centered Visit T"[b]))+ylab(expression("within-individual centered IVI"))+theme(text = element_text(size = 16))
# Figure2

#### Every 10th data point ####
Figure2.2 <- ggplot(data = Figure2_data_v3, aes(x = centered.visit.Tb, y = centered.IVI))+geom_point()+theme_classic()+xlab(expression("within-individual centered Visit T"[b]))+ylab(expression("within-individual centered IVI"))+theme(text = element_text(size = 16))
Figure2.2

# ggsave("Figure2.png", Figure2, width = 12, height = 8, dpi = 300, units = "in", device='png')
ggsave("Figure2.2.png", Figure2.2, width = 12, height = 8, dpi = 300, units = "in", device='png')

```

#### Figure 3. Effect of ambient temperature on IVI, visit body temperature, daily mean body temperature and daily mean ambient temperature.
```{r}
...
Figure3a <- ggplot(data = MCMC_model1_predict_df, aes(x = Avg.temp.C, y = median.visit.temperature.C))+geom_point()+theme_classic()+stat_smooth(aes(y = fit.visit.Tb.fit), method = "lm", se = FALSE, col = "red")+stat_smooth(aes(y = fit.visit.Tb.lwr), method = "lm", linetype = 2, se = FALSE, size = 0.75, col = "red")+stat_smooth(aes(y = fit.visit.Tb.upr), method = "lm", linetype = 2, se = FALSE, size = 0.75, col = "red")+ylab(expression("Visit T"[b] ~"(°C)"))+xlab(expression("Daily mean T"[a] ~"(°C)"))+labs(tag = "a")+theme(text = element_text(size = 16))
Figure3a

Figure3b <- ggplot(data = MCMC_model1_predict_df, aes(x = Avg.temp.C, y = IVI))+geom_point()+theme_classic()+stat_smooth(aes(y = fit.IVI.fit), method = "lm", se = FALSE, col = "red")+stat_smooth(aes(y = fit.IVI.lwr), method = "lm", linetype = 2, se = FALSE, size = 0.75, col = "red")+stat_smooth(aes(y = fit.IVI.upr), method = "lm", linetype = 2, se = FALSE, size = 0.75, col = "red")+ylab(expression("IVI (sec)"))+xlab(expression("Daily mean T"[a] ~"(°C)"))+labs(tag = "b")+theme(text = element_text(size = 16))
Figure3b
...
Figure3c <- ggplot(data = MCMC_model4_predict_df, aes(x = Avg.temp.C, y = mean.body.temperature))+geom_point()+theme_classic()+stat_smooth(aes(y = fit.daily.Tb.fit), method = "lm", se = FALSE, col = "red")+stat_smooth(aes(y = fit.daily.Tb.lwr), method = "lm", linetype = 2, se = FALSE, size = 0.75, col = "red")+stat_smooth(aes(y = fit.daily.Tb.upr), method = "lm", linetype = 2, se = FALSE, size = 0.75, col = "red")+ylab(expression("Daily mean T"[b] ~"(°C)"))+xlab(expression("Daily mean T"[a] ~"(°C)"))+labs(tag = "c")+theme(text = element_text(size = 16))
Figure3c

Figure3d <- ggplot(data = MCMC_model4_predict_df, aes(x = Avg.temp.C, y = total.feeder.visits))+geom_point()+theme_classic()+stat_smooth(aes(y = fit.daily.visits.fit), method = "lm", se = FALSE, col = "red")+stat_smooth(aes(y = fit.daily.visits.lwr), method = "lm", linetype = 2, se = FALSE, size = 0.75, col = "red")+stat_smooth(aes(y = fit.daily.visits.upr), method = "lm", linetype = 2, se = FALSE, size = 0.75, col = "red")+ylab(expression("Daily feeder visits"))+xlab(expression("Daily mean T"[a] ~"(°C)"))+labs(tag = "d")+theme(text = element_text(size = 16))
Figure3d
...
# Range is you exclude the two individuals with body temperature that were consistently lower than the rest.
...
# Generate plot
supp_plot1 <- ggplot(feeder14_birdvisits, aes(VisitDate, BirdNumbers)) + geom_point() + theme_classic() + ylab("Number of birds") + xlab("Date")
...
for (i in HEX.tag) 
{
  ...
  scale_color_manual(values = c("N" = "black","Y"="red"), name = "Spurious data point", labels = c("No", "Yes")) +
  facet_wrap(~Scan.Date) +
  ggtitle(paste0(i)) + xlab("Time") + ylab(expression("T"[b] ~"(°C)")))
...
### Additional plots for RE Peter Presentation
```{r}
# Within individual correlations between body temperature and daily feeder visits  
# Plotting with raw data
# Determine mean and sd for each individual and subtract from observed value
RE_Peter_Figure_data <- data.table(daily_thermal_bird_forage)

means.sd2 <- RE_Peter_Figure_data[, .(mean.daily.Tb = mean(mean.body.temperature, na.rm=TRUE), sd.daily.Tb = sd(mean.body.temperature, na.rm=TRUE) , mean.daily.feeder.visits = mean(total.feeder.visits, na.rm=TRUE), sd.daily.feeder.visits = sd(total.feeder.visits, na.rm=TRUE)), by= c("TransponderHexCode")] 

RE_Peter_Figure_data_v2 <- merge(RE_Peter_Figure_data, means.sd2, by = "TransponderHexCode")

RE_Peter_Figure_data_v2$centered.mean.Tb <- (RE_Peter_Figure_data_v2$mean.body.temperature-RE_Peter_Figure_data_v2$mean.daily.Tb)/RE_Peter_Figure_data_v2$sd.daily.Tb

RE_Peter_Figure_data_v2$centered.total.feeder.visits <- (RE_Peter_Figure_data_v2$total.feeder.visits-RE_Peter_Figure_data_v2$mean.daily.feeder.visits)/RE_Peter_Figure_data_v2$sd.daily.feeder.visits


RE_Peter_Figure1 <- ggplot(data = RE_Peter_Figure_data_v2, aes(x = centered.mean.Tb, y = centered.total.feeder.visits))+geom_point()+theme_classic()+xlab(expression("within-individual centered daily mean T"[b]))+ylab(expression("within-individual centered daily feeder visits"))+theme(text = element_text(size = 16))
RE_Peter_Figure1

ggsave("RE_Peter_Figure1.png", RE_Peter_Figure1, width = 12, height = 8, dpi = 300, units = "in", device='png')

# Among-individual correlations between daily mean body temperature and daily feeder visits
# Code adopted from the Houslay et al. 2017 tutorial 
df_Tb_feedervisits_coefs <- tibble(Trait = attr(colMeans(MCMC_model4$Sol), "names"),
Value = colMeans(MCMC_model4$Sol)) %>%
separate(Trait, c("Trait","Type1", "Type2", "Type3","TransponderHexCode"), sep = "\\.", fill = "right") %>%
filter(Type3 == "TransponderHexCode") %>%
unite(Trait, c(Trait, Type1, Type2), sep = "", remove = TRUE) %>%
spread(Trait, Value)

# remove Type3 column 
df_Tb_feedervisits_coefs_v2 <- df_Tb_feedervisits_coefs[,-1]

# Find the regression line -
# the covariance of boldness, relative fitness divided by
# the variance in boldness
Tb_feedervisits_slope <- MCMC_model4$VCV[,"traittotal.feeder.visits:traitmean.body.temperature.TransponderHexCode"]/
MCMC_model4$VCV[,"traitmean.body.temperature:traitmean.body.temperature.TransponderHexCode"]
ggplot(df_Tb_feedervisits_coefs_v2, aes(x = traitmeanbodytemperature, y = traittotalfeedervisits, group = TransponderHexCode)) +
geom_point(alpha = 0.7) +
geom_abline(intercept = 0, slope = mean(Tb_feedervisits_slope)) +
labs(x = "Daily mean body temperature (BLUP)",
y = "Daily feeder visits (BLUP)") +
theme_classic()
```
