...
#### After identifying spurious data visually, scroll through the data frame to find the detection and determining if the temperature record was a large jump in a short period of time (i.e., within a visit or between visits that occurred in short succession)
...
#### Figure 2. Effect of ambient temperature on IVI, visit body temperature, daily mean body temperature and daily foraging rate.

```{r}
## Figure 2ab. Effect of ambient temperature on IVI and visit body temperature
...
Figure2a <- ggplot(data = MCMC_model1_predict_df, aes(x = Avg.temp.C, y = median.visit.temperature.C)) +
  geom_point() +
  theme_classic() +
...
Figure2b <- ggplot(data = MCMC_model1_predict_df, aes(x = Avg.temp.C, y = IVI)) +
  geom_point() +
  theme_classic() +
...
Figure2c <- ggplot(data = MCMC_model4_predict_df, aes(x = Avg.temp.C, y = mean.body.temperature)) +
  geom_point() +
  theme_classic() +
...
Figure2d <- ggplot(data = MCMC_model4_predict_df, aes(x = Avg.temp.C, y = total.feeder.visits)) +
  geom_point() +
  theme_classic() +
  ...
  ylab(expression("Total daily feeder visits")) +
  xlab(expression("Daily mean T"[a] ~ "(°C)")) +
  labs(tag = "D") +
  theme(text = element_text(size = 16))
Figure2d
...
Figure2
ggsave("Figure2.tiff", Figure2, width = 13, height = 9, dpi = 300, units = "in", device = "tiff")
...
# Mean, SD and Range if you exclude the two individuals with body temperature that were consistently lower than the rest.
mean(daily_thermal_bird_forage_v2$mean.body.temperature, na.rm = TRUE)
sd(daily_thermal_bird_forage_v2$mean.body.temperature, na.rm = TRUE)
range(daily_thermal_bird_forage_v2$mean.body.temperature, na.rm = TRUE)
...
# Mean, SD and Range if you exclude the two individuals with body temperature that were consistently lower than the rest.
mean(thermal_bird_forage_v3$median.visit.temperature.C, na.rm = TRUE)
sd(thermal_bird_forage_v3$median.visit.temperature.C, na.rm = TRUE)
...
# Generate plot
FigureS2 <- ggplot(feeder14_birdvisits, aes(VisitDate, BirdNumbers)) +
  geom_point() +
  theme_classic() +
  ylab("Number of birds") +
  xlab("Date")+
  scale_x_date(limits = as.Date(c("2022-12-01", "2023-03-17")),date_breaks = "2 week", date_labels = "%b %d")
...
FigureS2_v2 <- FigureS2 + geom_vline(xintercept = as.numeric(as.Date(c("2023-01-10", "2023-03-01"))), linetype = c("dashed", "dotdash"), linewidth = 1.2, col = c("#1E88E5", "#FFA824"))
...
FigureS2_v3 <- FigureS2_v2 + geom_rect(data = shade, aes(xmin = min(VisitDate), xmax = max(VisitDate), ymin = -Inf, ymax = Inf), alpha = 0.1, fill = "#D81B60") + theme(text = element_text(size = 14))
FigureS2_v3

ggsave(FigureS2_v3, file = "FigureS2.png", width = 30, height = 15, units = "cm")
...
FigureS3_data <- Tb_data_v3
FigureS3_data$Time <- as.POSIXct(Tb_data_v3$Scan.Time, format = "%H:%M:%S")

for (i in HEX.tag)
{
  HEX.tag.records <- subset(FigureS3_data, HEX.Tag.ID == i)
  ...
  ggsave(plot, file = paste0("FigureS3_", i, ".png"), width = 35, height = 15, units = "cm")
  # scatter plot of each temperature measure vs time within each day
}
```

#### Figure 4S. Within individual correlations
...
FigureS4a_data <- data.table(thermal_bird_forage_v2)

means.sd <- FigureS4a_data[, .(mean.visit.Tb = mean(median.visit.temperature.C, na.rm = TRUE), sd.visit.Tb = sd(median.visit.temperature.C, na.rm = TRUE), mean.IVI = mean(IVI, na.rm = TRUE), sd.IVI = sd(IVI, na.rm = TRUE)), by = c("TransponderHexCode")]

FigureS4a_data_v2 <- merge(FigureS4a_data, means.sd, by = "TransponderHexCode")

# Standardize visit body temperature and IVI
FigureS4a_data_v2$centered.visit.Tb <- (FigureS4a_data_v2$median.visit.temperature.C - FigureS4a_data_v2$mean.visit.Tb) / FigureS4a_data_v2$sd.visit.Tb

FigureS4a_data_v2$centered.IVI <- (FigureS4a_data_v2$IVI - FigureS4a_data_v2$mean.IVI) / FigureS4a_data_v2$sd.IVI
...
FigureS4a_data_v3 <- FigureS4a_data_v2 %>%
  slice(sequence)

#### All data points ####
# FigureS4a <- ggplot(data = FigureS4a_data_v2, aes(x = centered.visit.Tb, y = centered.IVI))+geom_point()+theme_classic()+xlab(expression("within-individual centered Visit T"[b]))+ylab(expression("within-individual centered IVI"))+theme(text = element_text(size = 16))
# FigureS4a

#### Every 10th data point ####
FigureS4.2a <- ggplot(data = FigureS4a_data_v3, aes(x = centered.visit.Tb, y = centered.IVI)) +
  geom_point() +
  theme_classic() +
...
FigureS4b_data <- data.table(daily_thermal_bird_forage)

means.sd2 <- FigureS4b_data[, .(mean.daily.Tb = mean(mean.body.temperature, na.rm = TRUE), sd.daily.Tb = sd(mean.body.temperature, na.rm = TRUE), mean.daily.feeder.visits = mean(total.feeder.visits, na.rm = TRUE), sd.daily.feeder.visits = sd(total.feeder.visits, na.rm = TRUE)), by = c("TransponderHexCode")]

FigureS4b_data_v2 <- merge(FigureS4b_data, means.sd2, by = "TransponderHexCode")

FigureS4b_data_v2$centered.mean.Tb <- (FigureS4b_data_v2$mean.body.temperature - FigureS4b_data_v2$mean.daily.Tb) / FigureS4b_data_v2$sd.daily.Tb

FigureS4b_data_v2$centered.total.feeder.visits <- (FigureS4b_data_v2$total.feeder.visits - FigureS4b_data_v2$mean.daily.feeder.visits) / FigureS4b_data_v2$sd.daily.feeder.visits


FigureS4b <- ggplot(data = FigureS4b_data_v2, aes(x = centered.mean.Tb, y = centered.total.feeder.visits)) +
  geom_point() +
  theme_classic() +
  xlab(expression("Within-individual centered daily mean visit T"[b])) +
  ylab(expression("Within-individual centered total daily feeder visits")) +
  theme(text = element_text(size = 16)) +
  labs(tag = "b") 
FigureS4b
...
# Combine panels into one figure
FigureS4 <- ggarrange(FigureS4.2a, FigureS4b, ncol = 2, nrow = 1)
FigureS4
ggsave("FigureS4.png", FigureS4, width = 15, height = 7, dpi = 300, units = "in", device = "png")
```
